...
def algo_miss_featuresX(train_x, label, miss_column, algorithm, file_name, label_field=0):
    ...
    for en_size in range(1, ensemble_size):
        ...
        for alpha in miss_prop:
            for num_missing in range(1, fraction_missing_features):
                ...
                #print alpha, num_missing
                result = benchmarks(train_x, label, miss_column, algorithm, alpha, num_missing)
    ...
    #
    # resultp = Parallel(n_jobs=num_cores)(delayed(benchmarks)(train_x, label,miss_column, algorithm,alpha, num_miss)
    #                                      for alpha in miss_prop for num_miss in fraction_missing_features)
    # print time.time() - start
    return result
